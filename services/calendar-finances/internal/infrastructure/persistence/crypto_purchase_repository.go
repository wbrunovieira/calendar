...
func (r *CryptoPurchaseRepository) Create(cp *cryptopurchase.CryptoPurchase) error {
	query := `INSERT INTO finance.crypto_purchases
		(transaction_id, bank_account_id, profile_id, asset, quantity, price_usd, exchange_rate, invested_brl, invested_usd, occurred_on, notes)
...
func (r *CryptoPurchaseRepository) FindByProfileID(profileID string) ([]*cryptopurchase.CryptoPurchase, error) {
	return r.query(`SELECT id, transaction_id, bank_account_id, profile_id, asset, quantity, price_usd, exchange_rate, invested_brl, invested_usd, occurred_on, notes, created_at
...
func (r *CryptoPurchaseRepository) FindByTransactionID(transactionID string) (*cryptopurchase.CryptoPurchase, error) {
	results, err := r.query(`SELECT id, transaction_id, bank_account_id, profile_id, asset, quantity, price_usd, exchange_rate, invested_brl, invested_usd, occurred_on, notes, created_at
...
func (r *CryptoPurchaseRepository) query(q string, args ...interface{}) ([]*cryptopurchase.CryptoPurchase, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&cp.InvestedBRL, &cp.InvestedUSD, &cp.OccurredOn, &cp.Notes, &cp.CreatedAt,
		); err != nil {
			return nil, err
		}
