...
func TestAddAmount(t *testing.T) {
	t.Run("should add amount to open invoice", func(t *testing.T) {
		inv := createTestInvoice(StatusOpen)

		err := inv.AddAmount(100.50)

		if err != nil {
			t.Errorf("Expected no error, got %v", err)
		}
		if inv.Amount != 100.50 {
			t.Errorf("Expected amount 100.50, got %f", inv.Amount)
		}
	})

	t.Run("should accumulate amounts", func(t *testing.T) {
		inv := createTestInvoice(StatusOpen)

		inv.AddAmount(100.00)
		inv.AddAmount(50.50)

		if inv.Amount != 150.50 {
			t.Errorf("Expected amount 150.50, got %f", inv.Amount)
		}
	})

	t.Run("should return error when invoice is closed", func(t *testing.T) {
		inv := createTestInvoice(StatusClosed)

		err := inv.AddAmount(100.00)
...
func TestSubtractAmount(t *testing.T) {
	t.Run("should subtract amount from open invoice", func(t *testing.T) {
		inv := createTestInvoice(StatusOpen)
		inv.Amount = 200.00

		err := inv.SubtractAmount(50.00)

		if err != nil {
			t.Errorf("Expected no error, got %v", err)
		}
		if inv.Amount != 150.00 {
			t.Errorf("Expected amount 150.00, got %f", inv.Amount)
		}
	})

	t.Run("should subtract amount from closed invoice", func(t *testing.T) {
		inv := createTestInvoice(StatusClosed)
		inv.Amount = 200.00
