...
export default function StocksTab() {
  ...
  if (loading) {
    return <div className="text-center py-12 text-white/60">Carregando dados do mercado...</div>;
  }
  ...
  return (
    <>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
        <div className="flex flex-wrap items-center gap-6">
          <div className="flex items-center gap-2">
            ...
            >
              {[5, 10, 15, 20, 30].map((n) => (
                <option key={n} value={n} className="bg-gray-800">{n} acoes</option>
              ))}
            </select>
          </div>
          <div className="flex items-center gap-2">
          ...
          {recommendationDate && (
            <span className="text-white/50 text-sm">Dados de: {recommendationDate}</span>
          )}
          <span className="text-white/50 text-sm">{filteredStocks.length} de {stocks.length} acoes</span>
        </div>
      </div>

      {/* Recommendations Table */}
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
        <h3 className="text-lg font-semibold text-white mb-4">Top {topN} Acoes - Magic Formula</h3>
                ...
                >
                  <td className="py-3 pr-4">
                    <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold ${
                      stock.rank <= 3 ? 'bg-amber-500/20 text-amber-300' :
                      stock.rank <= 5 ? 'bg-emerald-500/20 text-emerald-300' :
                      'bg-white/10 text-white/60'
                    }`}>{stock.rank}</span>
                  </td>
                  <td className="py-3 pr-4 text-white font-semibold">{stock.ticker}</td>
                  <td className="py-3 pr-4 text-right text-white/80">{formatCurrency(stock.price)}</td>
                  <td className="py-3 pr-4 text-right text-emerald-400">{(stock.ebit_ev * 100).toFixed(1)}%</td>
                  <td className="py-3 pr-4 text-right text-blue-400">{(stock.roic * 100).toFixed(1)}%</td>
                  <td className="py-3 pr-4 text-right text-white/60">{stock.ranking_ebit_ev}</td>
                  <td className="py-3 pr-4 text-right text-white/60">{stock.ranking_roic}</td>
      ...
      {backtestChartData.length > 0 && (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          <h3 className="text-lg font-semibold text-white mb-2">Backtest: Magic Formula vs Ibovespa</h3>
          <p className="text-white/50 text-xs mb-4">Retorno acumulado (%) ao longo do tempo</p>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={backtestChartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                ...
                <XAxis dataKey="date" tick={{ fill: '#94a3b8', fontSize: 10 }} interval={Math.floor(backtestChartData.length / 8)} />
                <YAxis tick={{ fill: '#94a3b8', fontSize: 12 }} tickFormatter={(v) => `${v}%`} />
                <Tooltip content={<CustomTooltip />} />
                <Legend />
                <Area type="monotone" dataKey="magic_formula" name="Magic Formula" stroke="#10b981" fill="url(#colorMF)" strokeWidth={2} />
                <Area type="monotone" dataKey="ibovespa" name="Ibovespa" stroke="#3b82f6" fill="url(#colorIbov)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          {backtest && (
            <div className="mt-4 flex gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white/70 text-sm">
                  Magic Formula:{' '}
                  <span className={`font-semibold ${backtest.magic_formula[backtest.magic_formula.length - 1] >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-white/70 text-sm">
                  Ibovespa:{' '}
                  <span className={`font-semibold ${backtest.ibovespa[backtest.ibovespa.length - 1] >= 0 ? 'text-blue-400' : 'text-rose-400'}`}>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
        ...
        {loadingHistory && <div className="text-white/60 text-sm py-4">Carregando historico...</div>}

        {rankingHistory && !loadingHistory && (
          <div>
            <div className="flex items-center gap-3 mb-4">
              <h4 className="text-white font-semibold">{rankingHistory.ticker}</h4>
              <span className="text-white/50 text-xs">{rankingHistory.history.length} registros</span>
            </div>

            <div className="h-64 mb-6">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={rankingHistory.history} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                  <XAxis dataKey="date" tick={{ fill: '#94a3b8', fontSize: 10 }} interval={Math.floor(rankingHistory.history.length / 6)} />
                  <YAxis reversed tick={{ fill: '#94a3b8', fontSize: 12 }} domain={[1, 'auto']} label={{ value: 'Ranking', angle: -90, position: 'insideLeft', fill: '#94a3b8', fontSize: 12 }} />
                  <Tooltip
                    content={({ active, payload, label }) => {
                      if (active && payload && payload.length) {
                        ...
                        return (
                          <div className="bg-gray-900/95 border border-white/20 rounded-lg p-3 shadow-xl">
                            <p className="text-white/80 text-sm font-medium mb-1">{label}</p>
                            <p className="text-sm text-amber-300">Ranking: #{d.rank}</p>
                            <p className="text-sm text-emerald-400">EBIT/EV: {(d.ebit_ev * 100).toFixed(1)}%</p>
                            <p className="text-sm text-blue-400">ROIC: {(d.roic * 100).toFixed(1)}%</p>
                            <p className="text-sm text-white/70">Preco: {formatCurrency(d.price)}</p>
                            {d.in_top10 && <p className="text-sm text-emerald-300 font-semibold mt-1">Top 10</p>}
                          </div>
                        );
                      }
                      return null;
                  ...
                  <Line
                    ...
                    dot={(props) => {
                      ...
                      return (
                        <circle key={`${cx}-${cy}`} cx={cx} cy={cy} r={inTop10 ? 5 : 3} fill={inTop10 ? '#10b981' : '#f59e0b'} stroke={inTop10 ? '#10b981' : '#f59e0b'} strokeWidth={1} />
                      );
                    }}
                  />
                  <Line type="monotone" dataKey={() => 10} stroke="#10b981" strokeWidth={1} strokeDasharray="5 5" dot={false} name="Top 10" />
                </LineChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="h-48">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={rankingHistory.history} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                  <defs>
